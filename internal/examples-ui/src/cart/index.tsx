...
export const Cart: FC<PropsWithChildren<Cart>> = ({
   ...
   const cart = useMemo(() => {
      return products.reduce<Array<Product & { quantity: number }>>(
         (acum, current) => {
            const productIndex = acum.findIndex((_product) => _product.id === current.id)
         ...
         <DropdownMenu.Portal>
            <StlyedDropdownMenuContent sideOffset={12} align='end'>
               ...
               <StyledCartProductList>
                  {cart.length < 1
                     ? <p style={{ textAlign: 'center', fontSize: '1.4rem', margin: '48px 0' }}>Your Cart is Empty</p>
                     : (cart.map((product, index) => (
                        <StyledDropdownMenuCartProduct key={index}>
                           <StyledCartProductThumbnail>
                              <img src={product.image.src} alt={product.image.alt} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                           </StyledCartProductThumbnail>
                           <StyledProductBody>
                              <div>
                              ...
                              <StyledProductControls>
                                 <div style={{ display: 'flex', alignItems: 'center' }}>
                                    ...
                                    <p style={{ margin: '0 12px 0 0' }}>{product.quantity}</p>
                                    <StyledDropdownMenuItemProductControls
                                       onSelect={(e: any) => {
                                          e.preventDefault();
                                          sumItem(product.id);
               ...
               <DropdownMenu.Separator style={{ height: 1, margin: '0 0 12px 0', backgroundColor: '#5c5c5c' }} />
               ...
               <StyledDropdownMenuGroupCartCheckout>
                  ...
                  <DropdownMenuItemCheckoutButton onSelect={checkout} disabled={cart.length < 1}>
                     Checkout
